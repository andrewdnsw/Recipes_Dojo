...
var fire_grab = function() {
  $("#dijit_layout_StackController_3_s_grab").click();
}; // запуск панели контекстного поиска
...
function spot_generator(spot_arr, spot_opened, deep) {
  ...
  do {
    random_index = Math.floor(
      (Math.random() * (spot_arr.length * 1000 - 1)) / 1000
    );
    var unic_index = false;
    var unic_x = true;
    for (var n = start; n < stop; n++) {
      if (random_index == spot_opened[n]) unic_x = false;
    }
...
(function() {
  ...
  var area,
    loader_gray =
      '<img class = "loaderGray" src="' +
      software_holder +
      'lodlive/img/ajax-loader-gray.gif" style="margin-left:6px;margin-top:2px"/><br>';
  var doAn_arr = [];
  var urlToAnnotate, anStopBtnPushed;
  var dojo_select_lodlive_dialog,
    dojo_select_wdqs_dialog,
    dojo_search_engines_dialog,
    dojo_select_context_dialog,
    dojo_select_show_dialog,
    dojo_select_sort_dialog,
    dojo_select_pareto_dialog,
    dojo_select_levenstain_dialog,
    site_url;
  var search_engines_dialog,
    search_engines_dialog_tt,
    context_dialog,
    context_dialog_tt,
    context_dialog_tt_id,
    show_dialog,
    show_dialog_tt,
    sort_dialog,
    sort_dialog_tt,
  ...
  var lbl_1,
    ...
    coming_search,
    req_text = space;
  var from_meta_keywords = "";
  var from_meta_categories = "";
  // опции поиска
  the_context = {};
  context_obj = {};
  tokens_obj = {};
  ...
  d_aspects = [];
  var max_aspects,
    select_context_tt_was = false;
  // panes - упорядоченный массив визуальных панелей и активных вкладок на этих панелях; обслуживается функций panes_stack(...)
  panes = [
    { id: "semantic", tab: "grab", z: 4 },
    { id: "global", tab: "web", z: 3 },
    { id: "docs", tab: "progr", z: 2 },
    { id: "libs", tab: "lib", z: 1 },
    { id: "frame", tab: "frame", z: 0 }
  ];
  ...
  function adjust_scroll() {
    make_fine.cssText =
      "position:absolute;top:10px;bottom:0px;left:12px;right:12px;font-size:13px;";
  ...
  function make_req_text(req_text, blogs, blogs_text) {
    var text = (blogs ? blogs_text + space + req_text : req_text).replace(
      reg_special,
      the_special
    );
    return (search_site ? "site:" + search_site + space + text : text).replace(
      reg_space,
      "+"
    );
  }
  ...
  engine_makers.Yahoo_task_constructor = function(
    search_type,
    make_snippets,
    machine
  ) {
    var x_pane = search_tabs[search_type].x_pane;
    var shippets_in_serp,
      appspot,
      search_machine_post_data = {};
    ...
    if (search_type == the_web)
      search_machine_post_data.search_engine_url =
        voice.yahoo_web_engine_url + addon;
    else if (search_type == "images")
      search_machine_post_data.search_engine_url =
        voice.yahoo_images_engine_url + addon;
    else if (search_type == "video")
      search_machine_post_data.search_engine_url =
        voice.yahoo_video_engine_url + addon;
    ...
    search_machine_post_data.request_text = make_req_text(
      req_text,
      self.blogs,
      "блог"
    );
    ...
    function get_8_yahoo(n) {
      // поиск окончен, принудительный выход из рекурсии
      if (!self.active) {
        machine_stop(search_type, machine);
        machine_done(search_type, machine, self.the_counter);
        return;
      } else if (
        n >= Number(options.search_engines.Yahoo) ||
        self.the_counter >= voice.mail_counter_max ||
        shippets_in_serp == 0
      ) {
        // поиск окончен, нормальный выход из рекурсии
        if (self.blogs) {
          // это поиск в блогах?
          search_machine_post_data.request_text = make_req_text(
            req_text,
      ...
      jqXHR.done(function(response) {
        ...
        if (reply_word == "normal") {
          ...
          var received_cookies = response.received_cookies
            .toString()
            .replace(reg_cookies, "")
            .replace(reg_space, "");
          ...
          if (received_cookies.indexOf("sSN") < 0)
            received_cookies += the_comma + voice.yahoo_sSN;
          search_machine_post_data.cookies = received_cookies; // установим свежие куки
          shippets_in_serp += make_snippets(
            JSON.parse(received_data),
            x_pane,
            search_type,
            search_tabs[search_type].had
          ); // делаем сниппеты
        ...
        } else if (reply_word == "denied") {
          // если Yahoo брыкается
          if (self.the_counter > voice.mail_counter_max) {
            // поисковая машина временно недоступна (исчерпан лимит запросов)
        ...
        } else if (reply_word == "error") {
          // Ошибка сервера, обнаружено исключение
          console.log(
            machine + the_defis + voice.server_err + response.exception
          );
          if (self.the_counter > voice.mail_counter_max) {
            // поисковая машина временно недоступна (исчерпан лимит запросов)
            machine_denied(search_type, machine);
      ...
      setTimeout(function() {
        // если вообще нет ответа от сервера
  ...
  engine_makers.MailRu_task_constructor = function(
    search_type,
    make_snippets,
    machine
  ) {
    ...
    search_machine_post_data.request_text = (search_site
      ? "site:" + search_site + space + req_text
      : req_text
    ).replace(reg_space, "+");
    search_machine_post_data.search_engine_name = machine;
    search_machine_post_data.search_type = search_type;
    if (search_type == the_web)
      search_machine_post_data.search_engine_url = voice.mailru_web_engine_url;
    else if (search_type == "images")
      search_machine_post_data.search_engine_url =
        voice.mailru_images_engine_url;
    else if (search_type == "video")
      search_machine_post_data.search_engine_url =
        voice.mailru_video_engine_url;
    else {
      this.active = false;
      empty_search(search_type, x_pane);
      return;
    ...
    if (document_cookie.indexOf("searchuid") < 0)
      document_cookie += the_comma + voice.mailru_searchuid;
    search_machine_post_data.cookies = document_cookie;
    ...
    function get_8_mailru(n, antirobot) {
      // поиск окончен, принудительный выход из рекурсии
      if (antirobot);
      else if (!self.active) {
        machine_stop(search_type, machine);
        ...
        n >= Number(options.search_engines.MailRu) ||
        self.the_counter >=
          Number(voice.mail_counter_max) 
      ) {
        // поиск окончен, нормальный выход из рекурсии
        self.active = false;
        machine_done(search_type, machine, self.the_counter);
      ...
      jqXHR.done(function(response) {
        ...
        if (reply_word == "normal") {
          ...
          if (received_cookies.indexOf("sid") < 0)
            received_cookies += the_comma + voice.mailru_searchuid;
          search_machine_post_data.cookies = received_cookies; // установим свежие куки
          if (ff) {
            // Браузер Mozilla Firefox (Gecko)
            //						var the_tab = window.open('','_blank');		// Контрольное окно
            if (search_type == "images" || search_type == "video") {
              // картинки обслужим во всплывающем окне (иначе не получается)
              the_tab = window.open(
                "",
                search_type,
                "left=9999,top=9999,width=50,height=50,location=no,directories=no,menubar=no,resizable=no,scrollbars=no,status=no,toolbar=no,alwaysRaised=no"
              );
                ...
                if (the_tab) {
                  ...
                  $(the_tab.document).ready(function() {
                    inner_timer = setInterval(function() {
                      js_result = the_tab.document.getElementById(
                        "mediaResponsesList"
                      );
            ...
            } else if (search_type == the_web) {
              // глобальный поиск обслужим в динамическом iframe, с изысками для FF
                ...
                .on("load", function() {
                  ...
                  timer = setInterval(function() {
                    ...
                    temp.innerHTML = $(iframe)
                      ...
                      .html();
                    if (
                      temp.innerHTML.indexOf("js-result") > -1 ||
                      temp.innerHTML.indexOf("mediaResponsesList") > -1
                    ) {
                      clearInterval(timer);
                      iframe_load = true;
                      firefox(temp);
              ...
              // остальные виды поиска не обслуживаем
              self.active = false;
              console.log(voice.s_err_serp);
              machine_done(search_type, machine, self.the_counter);
              empty_search(search_type, x_pane);
            ...
            var invisible_doc =
              invisible_iframe.contentDocument ||
              invisible_iframe.contentWindow.document;
            var z = response.serp
              ? response.serp
                  .replace(/window/g, "document")
        ...
        if (true) {
          setTimeout(function() {
            // если динамический iframe долго не загружается (ошибки Mail.ru?)
            if (!iframe_load) {
              ...
              appspot = spot_generator(
                voice.appspot_arr,
                mailru_appspot_opened,
                2
              ); // сменим прокси-сервер
              self.the_counter++;
        ...
        function firefox(doc) {
          ...
          if (doc.innerHTML)
            deny =
              doc.innerHTML.indexOf("antirobot") > -1
                ? true
                : doc.innerHTML.indexOf(voice.mailru_denied) > -1
                ? true
                : false;
          if (anti || deny) {
            if (self.the_counter > voice.mail_counter_max) {
            ...
            if (search_type == the_web) {
              dojo.query(".result__li", doc).forEach(function(item) {
                var a_title = dojo.query(
                  "h3.result__title a.light-link",
                  item
                )[0];
                if (a_title) {
                  var link = a_title.href;
                  if ($(a_title).hasClass("never-visited"))
                    link = link.replace(location.hostname, "go.mail.ru");
              ...
              dojo
                .query(".js-images-item.images-item", doc)
              ...
              dojo
                .query(".js-video-item.video-item", doc)
                .forEach(function(item) {
            ...
            self.shippets += shippets_in_serp;
            search_tabs[search_type].snippets += shippets_in_serp;
            if (response.request_text)
              console.log(
                machine + " - request_text = " + response.request_text
              );
  ...
  engine_makers.Yandex_task_constructor = function(
    search_type,
    make_snippets,
    machine
  ) {
    ...
    if (search_type == the_web) {
      ...
      search_machine_post_data.search_engine_url =
        voice.yandex_images_engine_url;
    } else if (search_type == "video") {
      search_machine_post_data.search_engine_url =
        voice.yandex_video_engine_url;
    } else {
      this.active = false;
      empty_search(search_type, x_pane);
      return;
    ...
    function get_8_yandex(n) {
      if (!self.active) {
        ...
        n >= Number(options.search_engines.Yandex) ||
        shippets_in_serp == 0
      ) {
        // поиск окончен, нормальный выход из рекурсии
        self.active = false;
      ...
      jqXHR.done(function(response) {
        ...
        if (reply_word == "normal") {
          shippets_in_serp = 0;
          var received_cookies = response.received_cookies
            ? response.received_cookies
                .toString()
                .replace(reg_cookies, "")
                .replace(reg_space, "")
            : voice.y_cookie_start;
          var received_data = response.response_data;
          normal_serp(search_type, machine, serp_number);
          //	Работа с куками, которые пришли с Google App Engine
          received_arr = received_cookies.split(",");
          if (received_cookies.indexOf("yandexuid") >= 0) {
            // найдем и сохраним свежий yandexuid
            for (var k = 0; k < received_arr.length; k++) {
              if (received_arr[k].indexOf("yandexuid") >= 0)
                yandexuid_last = received_arr[k].replace(reg_space, "");
            }
            search_machine_post_data.cookies = voice.y_cookie_start;
          } else {
            search_machine_post_data.cookies = voice.y_cookie_start;
          ...
          // если Яндекс брыкается
          if (self.the_counter > voice.y_counter_max) {
            // поисковая машина временно недоступна (исчерпан лимит запросов)
            machine_denied(search_type, machine);
            self.active = false;
            console.log(voice.s_err_limit);
  ...
  engine_makers.Google_task_constructor = function(
    search_type,
    make_snippets,
    machine
  ) {
    ...
    var shippets_in_serp,
      status,
      self = this; // внутренняя ссылка на объект
    self.shippets = 0; // счетчик сгенерированных сниппетов
    self.the_counter = 0; // счетчик запросов к Google Ajax Search
    var url_1 =
      voice.google_engine_url +
      search_type +
      "?v=1.0&key=" +
      voice.google_key +
      "&hl=ru&safe=active&callback=?&rsz=8&start=";
    var url_2 =
      "&q=" + (search_site ? "site:" + search_site + " " + req_text : req_text);
      ...
      var jqXHR = $.getJSON(url_1 + n + url_2) // запрос к поисковой машине
        .done(function(data) {
          ...
          if (
            ...
            Number(self.the_counter) >= Number(options.search_engines.Google)
          ) {
            if (!n) {
              self.active = false;
              machine_done(search_type, machine, self.the_counter);
  ...
  function tab_done(search_type) {
    console.log(
      "Всего сделано запросов - " +
        search_type +
        " - " +
        search_tabs[search_type].calls
    );
  }
  function machine_done(search_type, machine, the_counter) {
    console.log(
      machine +
        " - " +
        search_type +
  ...
  function ajax_fail(search_type, machine, status, readyState) {
    console.log(
      machine +
        " - $.ajax error: " +
        status +
  ...
  function empty_serp(search_type, machine, serp_number, appspot) {
    console.log(
      machine +
        " - запрос " +
        serp_number +
  ...
  function normal_serp(search_type, machine, serp_number) {
    console.log(
      machine +
        " - запрос " +
        serp_number +
  ...
  function machine_denied(search_type, machine) {
    console.log(machine + " - поисковая машина временно недоступна.");
  }
  function machine_start(search_type, machine, appspot) {
    console.log(
      machine + " - начат новый поиск " + search_type + the_defis + appspot
    );
  }
  function machine_stop(search_type, machine) {
    console.log(machine + " - работа принудительно закончена.");
  }
  ...
  function make_search(tab) {
    ...
    for (var k = 0; k < panes.length; k++) {
      if (panes[k].id == "global") {
        // найдем нужную панель
    ...
    if (!new_search) {
      // это переключение вкладок
      if (search_tabs[search_type].status) {
        if (search_tabs[search_type].status == "started")
          show_coming_search(search_type, x_pane);
        // на этой вкладке сейчас работают поисковые машины?
    ...
    for (var machine in options.search_engines) {
      if (Number(options.search_engines[machine]) > 0) {
        search_tabs[search_type].engines[machine] = new engine_makers[
          machine + "_task_constructor"
        ](
          search_type,
          snippets_makers["make_" + search_type + "_snippets"],
          machine
        );
      }
    }
  ...
  function fine_search(search_type, x_pane) {
    if (all_done(search_type)) {
      if ($("div:not(.search_forbidden)", $(x_pane)).is(":empty")) {
      } // есть ли сниппеты?
  ...
  function forbidden_search(search_type, x_pane) {
    if (all_done(search_type)) {
      ...
      $(x_pane).append(
        $("<div/>")
          .addClass("search_forbidden")
          .html(voice.forbidden)
      );
      $(x_pane).append($("<br/>"));
  ...
  function empty_search(search_type, x_pane) {
    if (all_done(search_type)) {
      ...
      if (search_tabs[search_type].snippets == 0)
        $(x_pane)
          .empty()
          .append(
            $("<div/>")
              .addClass("search_empty")
              .html(voice.not_found)
          );
  ...
  function stop_search_engines(search_type, x_pane) {
    for (var engine in search_tabs[search_type].engines)
      search_tabs[search_type].engines[engine].active = false;
    hide_coming_search(search_type);
  ...
  function show_coming_search(search_type, x_pane) {
    ...
    $(coming_search)
      ...
      .bind("mouseup", function() {
        stop_search_engines(search_type, x_pane);
      });
  }
  ...
  function alpha(x, y, the_aspect) {
    if (Number(x.getAttribute(the_aspect)) > Number(y.getAttribute(the_aspect)))
      return 1;
    else if (
      Number(x.getAttribute(the_aspect)) < Number(y.getAttribute(the_aspect))
    )
      return 0;
    else return null;
  }
  function d_y_x(_y, _x) {
    // чило аспектов, по которым _y превосходит _x
    var alpha_tempo,
      alpha_sum = 0;
  ...
  function web_snippets_dominance(x_pane, search_type) {
    var x_cur,
      ...
      x_previous_tempo,
      x_next,
      first = sort_attr_last.substr(0, 1);
    ...
    $(the_dot + search_type + "_snippet", $(x_pane)).each(function(i, element) {
      tempo_arr[i] = element;
    });
    for (var x = 0; x < tempo_arr.length; x++) {
      ...
      if (search_type == "images" || search_type == "video") {
        ...
        $anc.attr(
          "title",
          $anc.attr("title") + voice.domin + x_element_dominir
        );
      } else
        $(".rel_per", $x_element)[0].insertAdjacentHTML(
          "beforeEnd",
          voice.dominance + x_element_dominir
        );
      ...
      while (
        x_previous &&
        x_previous.hasAttribute("d") &&
        Number(x_previous.getAttribute("d")) <
          Number(x_element.getAttribute("d"))
      ) {
        x_cur = x_previous;
        x_previous = pSib(x_cur);
      }
    ...
    $(x_pane)
      .empty()
      .append($("<br/>")); // добавим пустую строку
    ...
    while (x_element) {
      ...
      while (
        ...
        Number(x_element.getAttribute("d")) ==
          Number(x_previous.getAttribute("d")) &&
        Number(x_element.getAttribute(first)) ==
          Number(x_previous.getAttribute(first)) &&
        Number(x_element.getAttribute(second)) >
          Number(x_previous.getAttribute(second))
      ) {
        x_cur = x_previous;
        x_previous = pSib(x_cur);
      }
  ...
  function put_snippet(x_div, x_pane) {
    if (!sort_attr || sort_attr == "n") {
      x_pane.insertBefore(x_div, x_pane.lastChild);
      return;
    }
    $(x_div)
      .attr("r", aspects.relevance)
    ...
    while (
      x_previous &&
      x_previous.hasAttribute(sort_attr_tempo) &&
      Number(x_previous.getAttribute(sort_attr_tempo)) <
        Number(x_div.getAttribute(sort_attr_tempo))
    ) {
      x_cur = x_previous;
      x_previous = pSib(x_cur);
    }
    ...
    if (sort_attr == "p" || sort_attr == "p_m") {
      while (
        ...
        Number(x_previous.getAttribute(sort_attr_tempo)) ==
          Number(x_div.getAttribute(sort_attr_tempo)) &&
        Number(x_previous.getAttribute("r")) < Number(x_div.getAttribute("r"))
      ) {
        x_cur = x_previous;
        x_previous = pSib(x_cur);
      }
      ...
      while (
        ...
        Number(x_previous.getAttribute(sort_attr_tempo)) ==
          Number(x_div.getAttribute(sort_attr_tempo)) &&
        Number(x_previous.getAttribute("p")) < Number(x_div.getAttribute("p"))
      ) {
        x_cur = x_previous;
        x_previous = pSib(x_cur);
      }
  ...
  snippets_makers.make_web_snippets = function(data, x_pane, search_type, had) {
    ...
    for (i in data.responseData.results) {
      // обработка порции фидов
      var x_data = data.responseData.results[i]; // очередной фид
      var x_title = x_data.titleNoFormatting
        .replace(reg_html, "")
        .replace(reg_quot, reg_quot_replace); // имя ресурса, очищенное от разметки
      x_data.content = x_data.content || x_title; // если content ресурса не задан
      var x_content = x_data.content
        .replace(reg_html, "")
        .replace(" Скрыть", ""); // content ресурса
      var x_url = x_data.unescapedUrl; // url ресурса
      var x_unic = x_url;
      var x_domain = reg_host.exec(x_data.unescapedUrl)[2]; // домен ресурса
      if (x_unic.indexOf(voice.y_images_search) > -1)
        x_unic = voice.y_images_search;
      if (x_unic.indexOf(voice.y_video_search) > -1)
        x_unic = voice.y_video_search;
      ...
      calc_aspects(
        x_title +
          ...
          x_content +
          space +
          x_domain.replace(reg_the_dot, space)
      ); // вычисление аспектов сниппета
      ...
      var x_hostname =
        '<div class="hostname">' +
        x_domain +
        '<div class="info rel_per">' +
        voice.relevance +
        (sort_attr == "r_m" ? aspects.relevance_meta : aspects.relevance) +
        voice.pertinence +
        (sort_attr == "p_m" ? aspects.pertinence_meta : aspects.pertinence);
  ...
  snippets_makers.make_images_snippets = function(
    data,
    x_pane,
    search_type,
    had
    ...
    function light_image(x_pane) {
      ...
      img.alt = "";
      img.style.cssText =
        "margin:0;vertical-align:middle;height:" +
        img_size +
        "px;width:" +
        img_size +
        "px;border-radius:" +
      ...
      x_div.style.cssText =
        "display:inline-block;border:3px solid gainsboro;margin:5px;border-radius:" +
        img_radius +
        "px;-webkit-border-radius:" +
        img_radius +
        "px;-moz-border-radius:" +
        img_radius +
        "px;box-shadow:4px 5px 3px gray;border-style:inset";
  ...
  snippets_makers.make_video_snippets = function(
    data,
    x_pane,
    search_type,
    had
    ...
    function light_video(x_pane) {
      ...
      var anchor = document.createElement("a");
      anchor.href =
        "http://www.youtube.com/embed/" + vid + "?rel=1&amp;wmode=transparent";
      anchor.title =
        x_title +
        the_dot +
        voice.relevan +
  ...
  function measure(obj_1, obj_2) {
    // свойствами каждого из obj_1 и obj_2 являются лексемы, их значениями - количество повторений данной лексемы
    var distance,
      count = 0;
    for (var name_1 in obj_1) {
      for (var name_2 in obj_2) {
        ...
        count +=
          (options.levenstain.equal - distance) * obj_1[name_1] * obj_2[name_2];
      }
    }
    return count;
  }
    ...
    for (i = -1, m = s.length; ++i < m; ) {
      for (j = -1, n = s2.length; ++j < n; ) {
        a[(i *= 1) + 1][(j *= 1) + 1] = Math.min(
          a[i][j + 1] + options.levenstain.remove,
          a[i + 1][j] + options.levenstain.insert,
          a[i][j] + (s[i] != s2[j] ? options.levenstain.replace : 0)
        );
      }
    }
    return a[l][t];
  ...
  function make_tokens_obj(obj, text) {
    ...
    for (var k = 0; k < arr.length; k++) {
      var x_token = arr[k];
      if (x_token.length > options.levenstain.max)
        x_token = x_token.substr(0, options.levenstain.max);
  ...
  panes_stack = function(pane, tab) {
    ...
    if (!arguments.length) {
      if (search_divs.indexOf(the_pane.id) > -1) {
        // если ушли с панелей поиска
        choice_menu_make(
          voice.choice_tuning,
          voice.choice_context,
          show_context_search
        );
        for (var k = 0; k < panes.length; k++) {
          the_pane = panes[k];
    ...
    if (the_pane.id == "global") {
      if (search_tabs[the_pane.tab].coming_search)
        coming_search.style.zIndex = 1;
  ...
  function view_tab(pane) {
    ...
    var skrepka = dojo.query(
      the_sharp + the_s + pane.tab + " > .skrepka_fixed"
    )[0];
    if (skrepka && skrepka.style.visibility == "visible") very_scroll(pane.tab);
    // скролл только панели результатов
    else {
      // скролл всей панели
  ...
  make_semantic = function(type) {
    ...
    if (type.indexOf("lodlive") > -1) {
      if (!lodlive_was) lodlive_load();
        ...
        setTimeout(function() {
          ...
          if (ff) {
            var iframe_doc =
              lodlive_frame.contentDocument ||
              lodlive_frame.contentWindow.document;
    ...
    } else if (type.indexOf("dbpedia") > -1) {
      if (!dbpedia_was) dbpedia_start(type);
      else if (!dbpedia_semantic) get_img($img_x, image_animator_x, 400, type);
    } else if (type.indexOf("wdqs") > -1) {
      if (!wdqs_was) wdqs_start(type);
      else if (!wdqs_semantic)
        get_img($img_wdqs, image_animator_wdqs, 400, type);
      else {
        var wdqs_frame = dojo.query("#wdqs > iframe")[0];
        wdqs_frame.style.width = "99%";
        setTimeout(function() {
          wdqs_frame.style.width = "100%";
        }, 9);
      }
    } else if (type.indexOf("annotation") > -1) {
      if (!annotation_was) annotation_start(type);
      else if (!annotation_semantic)
        get_img($img_y, image_animator_y, 400, type);
      else $("#annotation_queryText")[0].focus();
    } else if (type.indexOf(the_grab) > -1) {
  ...
  function start() {
    ...
    var $request = $(request);
    $request.val("").mouseover(function() {
      $request.unbind("focus");
      $request.focus();
    });
    request_clone = dojo.byId("request_clone");
    var $request_clone = $(request_clone);
    $request_clone.val("").mouseover(function() {
      $request_clone.unbind("focus");
      $request_clone.focus();
    });
    ...
    dojo.query(".demoLayout").forEach(function(item) {
      ...
      dojo.query(".centerPanel > div", item).forEach(function(s_pane) {
        if (pane_id == "docs") {
          $(dojo_prefix + "0_" + s_pane.id)
            .parent()
            .parent()
            .unbind("click")
            .bind("click", function() {
              panes_stack(pane_id, s_pane.id);
              do_search(true);
            });
        } else if (pane_id == "libs") {
          $(dojo_prefix + "1_" + s_pane.id)
            .parent()
            .parent()
            .unbind("click")
            .bind("click", function() {
          ...
          $(dojo_prefix + "2_" + s_pane.id)
            .parent()
            .parent()
            .unbind("click")
            .bind("click", function() {
              make_search(s_pane.id);
            });
        } else if (pane_id == "semantic") {
          $(dojo_prefix + "3_" + s_pane.id)
            .parent()
            .parent()
            .unbind("click")
            .bind("click", function() {
              make_semantic(s_pane.id);
            });
        }
      });
    });
    ...
    d_aspects = [];
    for (var aspect in options.pareto)
      if (options.pareto[aspect]) d_aspects.push(aspect); // выбор аспектов для показателя доминирования
    ...
    dojo.connect(request, "onchange", null, function() {
      request_clone.value = request.value;
    });
    dojo.connect(request_clone, "onchange", null, function() {
      request.value = request_clone.value;
    });
    dojo.connect(request, "onkeydown", null, function(e) {
      e = e || window.event;
      request_clone.value = request.value;
      if (e.keyCode == 13) do_search();
    });
    dojo.connect(request_clone, "onkeydown", null, function(e) {
      e = e || window.event;
      request.value = request_clone.value;
      if (e.keyCode == 13) do_search();
    });
    ...
    window.onbeforeunload = function() {
      ...
      document.cookie =
        ...
        ";"; // сохраним опции поиска options в куках
    };
    function show_help(url, name) {
      for (n = 0; n < win_opened.length; n++) {
        if (name == win_opened[n].name) {
          win_opened[n].win.close();
          win_opened.splice(n, 1);
        }
      }
      show_doc(url, name);
    }
    ...
    $("#back-top a")
      .unbind("mouseup")
      .bind("mouseup", function() {
        scroll_api.positionDragY(0, true);
        return false;
      });
  ...
  function do_search(in_tab) {
    for (var name in options.sort) {
      // повторная, контрольная установка опций сортировки сниппетов
      if (options.sort[name]) {
        sort_attr_last =
          sort_attr == "d" || sort_attr == "n" ? sort_attr_last : sort_attr;
        sort_attr = name;
      }
    }
    context_was = true;
    if (!request.value || request.value == "")
      req_text = request.value = request_clone.value = voice.ksst;

    var z = request.value; // нет очистки от знаков препинания и прочего
    req_text = request.value = z
      .replace(reg_union, space)
      .replace(reg_many_space, space); // очистка от союзов, местоимений и лишних пробелов
    ...
    search_site = null;
    if (search_selected == "wiki_en") {
      search_site = "en.wikipedia.org";
      make_search();
    } else if (search_selected == "wiki_ru") {
      search_site = "ru.wikipedia.org";
      make_search();
    } else make_search();
  }
  ...
  function doAnStartBtn() {
    ...
    area.value = "";
    if (!urlToAnnotate.length) {
      browserMessage(voice.noUrlToAnnotate);
      doAnStopBtn();
      return;
    }
    ...
    $anResult
      .empty()
      .html(
        '<label style="font-size:1.15em;">' + voice.an_result + "</label>" + br
      );
    for (var n = 0; n < urlToAnnotate.length; n++) {
      var url = urlToAnnotate[n].url;
      var d = reg_host.exec(url)[2]; // домен ресурса
      var $z = $(
        '<div class="an_link" url="' +
          url +
          '" hostname="' +
          d +
          '"><img src="http://favicon.yandex.net/favicon/' +
          d +
          '" style="border:0px;float:left;margin-right:5px;" alt="" url="' +
          url +
          '" hostname="' +
          d +
          '">' +
      ...
      $anResult.append($z);
      $anResult.append(
        $('<div id="anres_' + n + '">' + loader_gray + "</div>")
      );
  ...
  function get_access(store, item) {
    ...
    if (store.hasAttribute(item, "access")) {
      ...
      if (access == "0" || access == "common" || access == "public")
        doc_level = 0;
  ...
  function show_doc(src, name, text) {
    ...
    child = window.open(
      ...
      "left=" +
        ...
        ",height=" +
        height +
        ",location=no,directories=no,menubar=no,resizable=yes" +
        ",scrollbars=yes,status=no,toolbar=no,alwaysRaised=yes"
    );
    if (text) {
      var timer = setInterval(function() {
        if (child) {
          clearInterval(timer);
          if (child.document.body) child.document.body.innerHTML = "";
          child.document.writeln(text);
        }
      }, 10);
    }
    childIsOK(name);
  ...
  function load_doc(store, item) {
    ...
    var source =
      url == "" || url == null || url == undefined
        ? "http://vt.obninsk.ru/search_student/stock/no_doc.html"
        : url;
    var doc_level = get_access(store, item); // получим уровень защиты документу (из access)
    if (
      user_access_level >= doc_level ||
      user_name == "admin" ||
      (has_owner && user_name == store.getValue(item, "owner", null))
    );
    else {
      // есть доступ к документу
      // нет доступа к документу
      if (user_authorized != "true") source = stock_holder + "no_access.html";
      // не авторизован
      else if (doc_level == 2) source = stock_holder + "no_student.html";
      // документ для преподавателей
      else if (doc_level == 3) {
        // документ для членов проектных групп
        var access_atr = store.getValue(item, "access", null);
        if (access_atr.indexOf(user_name) == -1)
          source = stock_holder + "no_level.html";
      } else alert(voice.err); // какая-то ошибка
    }
    for (n = 0; n < win_opened.length; n++) {
      if (source == win_opened[n].name) {
        win_opened[n].win.close();
        win_opened.splice(n, 1);
      }
    }
    if (options.show.popup) show_doc(source, source);
    else if (options.show.tabs) {
      child = window.open(source, "_blank");
      childIsOK(source);
  ...
  function ajax_get(from, to) {
    dojo.xhrGet({
      ...
      error: function(response, ioArgs) {
        console.log(voice.ajaxError + ioArgs.xhr.status);
      }
    });
  }
  ...
  function test_browser() {
    if (
      ...
      ajax_get(
        stock_holder + "bad_semantic.html",
    ...
    dojo.xhrGet({
      ...
      load: function(result) {
        if (from.indexOf("form_navigation") >= 0) {
          ...
          form.appendChild(
            dojo.query("form#_footer_form_to_post", navigation)[0]
          );
          form_ok = true;
      ...
      error: function(response, ioArgs) {
        browserMessage(voice.ajaxError + ioArgs.xhr.status);
      }
    });
  }
  ...
  require([
    ...
    function initTree() {
      ...
      dojo.query("#make_fine .centerPanel.cloud > div").forEach(function(item) {
        ...
        trees[tab].treeStore = new dojo.data.ItemFileWriteStore({
          url: cloud_holder + tab + ".json"
        });
    ...
    dojo.ready(function() {
      ...
      if (test_browser()) {
        ...
        var timer = setInterval(function() {
          if (form_ok && voice_ok && context_dialog_ok) {
            clearInterval(timer);
            $.getScript(software_holder + "script/add_engine.js", start);
          } // добавим ещё немного скрипта
        }, 10);
      }
    });
  });
  // Случайное изображение, без повторов (переписать адрес для изображений или совсем убрать)
  function get_img($img, image_animator, delay, type) {
    ...
    $img.unbind("load").bind("load", function() {
      image_animator.stop().play(delay);
      setTimeout(function() {
        $img.css("display", "block");
      }, delay);
    });
    $img.css("cursor", "pointer");
    if (type.indexOf("dbpedia") > -1) {
      $img.unbind("mouseup").bind("mouseup", dbpedia_load);
      $img.attr("title", voice.mes_dbpedia_go);
    } else if (type.indexOf("wdqs") > -1) {
      $img.unbind("mouseup").bind("mouseup", wdqs_load);
      $img.attr("title", voice.mes_wdqs_go);
    } else if (type.indexOf("annotation") > -1) {
      $img.unbind("mouseup").bind("mouseup", annotation_load);
      $img.attr("title", voice.mes_annotation_go);
    } else if (type.indexOf(the_grab) > -1) {
      $img.unbind("mouseup").bind("mouseup", function() {
        do_search(null);
      });
      $img.attr("title", voice.mes_grab_go);
      is_grab = true;
    }
    ...
    if (is_grab) {
      do {
        random = Math.floor(
          (Math.random() * (max_images_is_grab * 1000 - 1)) / 1000
        );
        unic_img = false;
        unic_x = true;
        for (
          var n = img_opened_is_grab.length;
          n > 0 && n > img_opened_is_grab.length - 3;
          n--
        ) {
          if (random == img_opened_is_grab[n - 1]) {
            unic_x = false;
            break;
          }
        }
      ...
      do {
        random = Math.floor(
          max_images_is_grab +
            (Math.random() * (max_images_no_grab * 1000 - 1)) / 1000
        );
        unic_img = false;
        unic_x = true;
        for (
          var n = img_opened_no_grab.length;
          n > 0 && n > img_opened_no_grab.length - 2;
          n--
        ) {
          if (random == img_opened_no_grab[n - 1]) {
            unic_x = false;
            break;
          }
        }
  ...
  function activate_frame(url, host) {
    ...
    if (hostname.indexOf("youtu") >= 0) {
      // youtube покажем через proxy, хотя всё равно может на сработать ...
    ...
    dojo.xhrGet({
      ...
      load: function(result) {
        if (result.indexOf("true") > -1) {
          // сайт не разрешает открывать себя в iframe
                ...
                try {
                  ...
                  $(iframe).load(function() {
                    ...
                    if (iframe_doc.body == "") bad_site();
                      ...
                      console.log(
                        "Сайт подгружен в iframe через pythongrab: " + url
                      );
                  });
                } catch (e) {
                  console.log(
  ...
  function on_mouse_up(event) {
    ...
    if (
      user_access_level >= doc_level ||
      user_name == "admin" ||
      (has_owner && user_name == target.getAttribute("owner"))
    );
    else {
      // есть доступ к документу
      // нет доступа к документу
      if (user_authorized != "true") url = stock_holder + "no_access.html";
      // не авторизован
      else if (doc_level == 2) url = stock_holder + "no_student.html";
      // документ для преподавателей
      else if (doc_level == 3) {
        // документ для членов проектных групп
        var access = target.getAttribute("access");
        if (access.indexOf(user_name) == -1)
          url = stock_holder + "no_level.html";
      } else alert(voice.err); // какая-то ошибка
    }
    for (n = 0; n < win_opened.length; n++)
      if (url == win_opened[n].name) {
        win_opened[n].win.close();
        win_opened.splice(n, 1);
      }
  ...
  function fetch_tree(tab, store, req) {
    ...
    phrase = phrase.replace(
      new RegExp(" и | или | для | and | or | for "),
      space
    );
          ...
          onComplete: function(items) {
            ...
            dojo.forEach(items, function(item) {
              ...
              if (!store.hasAttribute(item, "children") && the_url != "") {
                ...
                var the_access =
                  store.getValue(item, "access", null) || "common";
                the_feed = {
                  unescapedUrl: the_url,
                  titleNoFormatting: the_title
                };
                the_feed.content =
                  voice.keywords +
                  (the_keywords ? the_keywords : voice.unknown) +
                  the_dot +
                  space +
                  ru_access(the_access);
                if ($.inArray(the_feed, feeds.responseData.results) < 0)
                  feeds.responseData.results.push(the_feed);
                links++;
              }
            });
  ...
  function pSib(obj) {
    while ((obj = obj.previousSibling)) if (obj.nodeType == 1) return obj;
    return null;
  }
  function pNext(obj) {
    while ((obj = obj.nextSibling)) if (obj.nodeType == 1) return obj;
    return null;
  }
  ...
  function select_search_engines(visible) {
    if (!dojo_search_engines_dialog)
      dojo_search_engines_dialog = new dojo_dialog_constructor({
        title: voice.select_search_engines,
        content: search_engines_dialog
      });
    for (var aspect in options.search_engines) {
      // по всем аспектам
      dojo.query(
        the_sharp + aspect + "_number",
        search_engines_dialog
      )[0].value = options.search_engines[aspect];
    }
    if (visible) dojo_search_engines_dialog.show();
    else
      dojo
        .query("button", dojo_search_engines_dialog.content)
        .forEach(function(item) {
          item.onmouseup = select_search_engines_done;
        });
    function select_search_engines_done(e) {
      ...
      if (target.id == "select_search_engines_done") {
        for (var aspect in options.search_engines) {
          // сохраним в options сделанный выбор
          options.search_engines[aspect] = dojo.query(
            the_sharp + aspect + "_number",
            search_engines_dialog
          )[0].value;
        }
      } else if (target.id == "refresh_search_options") {
        var z = options.lodlive;
        options = eval("(" + voice.options + ")");
        options.lodlive = z;
      } // восстановим значения options по умолчанию
    ...
    if (visible) {
      ...
      site_url = dojo.query(
        the_sharp +
          ...
          space +
          the_sharp +
          "handle_site_upload_text"
      )[0];
      if (site_url)
        dojo.query(
          "input#handle_site_upload_text",
          dojo_select_context_dialog.content
        )[0].value = site_url.value;
      var _files = dojo.query(
        the_sharp +
          context_dialog_tt_id +
          space +
          the_sharp +
      ...
      if (_files)
        dojo.query(
          ...
          dojo_select_context_dialog.content
        )[0].files = _files.files;
    } else {
      $("span.dijitDialogCloseIcon").bind("mouseup", alert_hide);
      dojo.query(
        "button#select_context_close",
        dojo_select_context_dialog.content
      )[0].onmouseup = hide;
      dojo.query(
        "button#select_context_done",
        dojo_select_context_dialog.content
      )[0].onmouseup = function() {
        select_context_done();
        hide();
      };
      dojo.query(
        "input#handle_site_upload",
        dojo_select_context_dialog.content
      )[0].onmouseup = function() {
        handle_site_upload(true, this);
      };
      dojo.query(
        "input#handle_site_upload_text",
        dojo_select_context_dialog.content
        ...
        e = e || event;
        if (e.keyCode == 13) handle_site_upload(true, this);
      };
      dojo.query(
        "input#handle_file_upload",
        dojo_select_context_dialog.content
      )[0].onchange = function() {
        handle_file_upload(this.files, this);
      };
      //			select_context_done();
    }
    function hide() {
  ...
  function select_context_start(key) {
    var $p, $eye, _cbx, $_cbx;
    for (var name in options.context) {
      // по всем вариантам задания контекста
      if (key)
        _cbx = dojo.query(
          the_sharp + name + "_cbx",
          dojo_select_context_dialog.content
        )[0];
      // ищем в верхнем диалоге
      else
        _cbx = dojo.query(
          the_sharp + context_dialog_tt_id + space + the_sharp + name + "_cbx"
        )[0]; // ищем в тултипе, по id
      ...
      if (name == "file" || name == "web") {
        $eye.attr("src", voice.eye);
      }
      if (_cbx.checked) {
        // если выбран данный вид контекста
  ...
  function cbx_change($_cbx) {
    $_cbx.unbind().change(function(e) {
      // обработчик событий ВКЛ/ВЫКЛ
      e = e || window.event;
      var target = e.target || e.srcElement;
      var id = target.id;
      var name = id.substr(0, id.indexOf("_"));
  ...
  function select_context_done() {
    context_obj = {};
    for (var name in options.context) {
      // формируем context_obj, пройдя по всем вариантам задания контекста
      if (options.context[name] && the_context[name]) {
        var text = the_context[name]
          .toLowerCase()
          .replace(reg_punct, " ")
          .replace(reg_union, " ")
          .replace(/[\r\n\f]/g, " ");
      ...
      var jqXHR = $.get(voice.grab + "?url=" + z + ";cleaning=true") // запрос к сайту через прокси-сервер pythongrab
        .done(function(data) {
          ...
          if (data == "") {
            ...
            alert(voice.bad_url + ". URL: " + z);
          } else {
            the_context[name] = data;
            eye.src = "http://unicorn.kl.com.ua/search_student/images/_show.png";
          }
        })
        .fail(function(obj, status) {
          eye.style.display = "none";
  ...
  function handle_file_upload(files, obj) {
    ...
    var jqXHR = $.ajax({
      ...
      .done(function(data) {
        ...
        if (
          _p.parentNode.parentNode.parentNode.className ==
  ...
  function handle_site_upload(key, obj) {
    ...
    if (key) {
      // ищем в верхнем диалоге
      the_img = dojo.query(
        "img#select_context_site_upload",
        dojo_select_context_dialog.content
      )[0];
      site_url = dojo.query(
        "input#handle_site_upload_text",
      ...
      // ищем в тултипе, по id
      the_img = dojo.query(
        the_sharp +
          context_dialog_tt_id +
          space +
          the_sharp +
          "select_context_site_upload"
      )[0];
      site_url = dojo.query(
        the_sharp +
          context_dialog_tt_id +
  ...
  function select_sort(visible) {
    if (!dojo_select_sort_dialog)
      dojo_select_sort_dialog = new dojo_dialog_constructor({
        title: voice.select_sort,
        content: sort_dialog
      });
    for (var name in options.sort) {
      // по всем вариантам
      dojo.query("#_sort_dialog " + the_sharp + name + "_radio")[0].checked =
        options.sort[name];
    }
    if (visible) dojo_select_sort_dialog.show();
    else
      dojo
        .query("button", dojo_select_sort_dialog.content)
        .forEach(function(item) {
          item.onmouseup = select_sort_done;
        });
    function select_sort_done(e) {
      ...
      if (target.id == "select_sort_done") {
        for (var name in options.sort) {
          // сохраним сделанный выбор
          options.sort[name] = dojo.query(
            "#_sort_dialog " + the_sharp + name + "_radio"
          )[0].checked;
          console.log(
            "select_sort_done: name = " +
              name +
              " value = " +
              options.sort[name]
          );
  ...
  function select_show(visible) {
    if (!dojo_select_show_dialog)
      dojo_select_show_dialog = new dojo_dialog_constructor({
        title: voice.select_show,
        content: show_dialog
      });
    for (var name in options.show) {
      // по всем вариантам
      dojo.query("#_show_dialog " + the_sharp + name + "_radio")[0].checked =
        options.show[name];
    }
    if (visible) dojo_select_show_dialog.show();
    else
      dojo
        .query("button", dojo_select_show_dialog.content)
        .forEach(function(item) {
          item.onmouseup = select_show_done;
        });
    function select_show_done(e) {
      ...
      if (target.id == "select_show_done") {
        for (var name in options.show) {
          // сохраним сделанный выбор
          options.show[name] = dojo.query(
            "#_show_dialog " + the_sharp + name + "_radio"
          )[0].checked;
        }
      }
  ...
  function select_pareto(visible) {
    if (!dojo_select_pareto_dialog)
      dojo_select_pareto_dialog = new dojo_dialog_constructor({
        title: voice.select_pareto,
        content: pareto_dialog
      });
    for (var aspect in options.pareto) {
      // по всем аспектам
      dojo.query("#_pareto_dialog " + the_sharp + aspect + "_cbx")[0].checked =
        options.pareto[aspect];
    }
    if (visible) dojo_select_pareto_dialog.show();
    else
      dojo
        .query("button", dojo_select_pareto_dialog.content)
        .forEach(function(item) {
          item.onmouseup = select_pareto_done;
        });
    function select_pareto_done(e) {
      ...
      if (target.id == "select_pareto_done") {
        ...
        for (var aspect in options.pareto) {
          // сохраним сделанный выбор
          options.pareto[aspect] = dojo.query(
            "#_pareto_dialog " + the_sharp + aspect + "_cbx"
          )[0].checked;
  ...
  function select_levenstain(visible) {
    if (!dojo_select_levenstain_dialog)
      dojo_select_levenstain_dialog = new dojo_dialog_constructor({
        title: voice.select_levenstain,
        content: levenstain_dialog
      });
    for (var aspect in options.levenstain) {
      // по всем аспектам
      dojo.query(
        "#_levenstain_dialog " + the_sharp + aspect + "_number"
      )[0].value = options.levenstain[aspect];
    }
    if (visible) dojo_select_levenstain_dialog.show();
    else
      dojo
        .query("button", dojo_select_levenstain_dialog.content)
        .forEach(function(item) {
          item.onmouseup = select_levenstain_done;
        });
    function select_levenstain_done(e) {
      ...
      if (target.id == "select_levenstain_done") {
        for (var aspect in options.levenstain) {
          // сохраним сделанный выбор
          options.levenstain[aspect] = dojo.query(
            "#_levenstain_dialog " + the_sharp + aspect + "_number"
          )[0].value;
        }
      }
  ...
  function grab_start(type) {
    ...
    image_animator_z = new anim({
      node: $img_z.get(0),
      easing: backOut,
      properties: {
        marginTop: { start: 960, end: 30 },
    ...
    require(["dijit/TooltipDialog", "dojo/on"], function(TooltipDialog, on) {
      ...
      require(["dijit/popup"], function(popup) {
        var target,
          menu_item = dojo.byId("the_search_engines");
        ...
        var select_search_engines_tt = new TooltipDialog({
          ...
          onMouseLeave: function() {
            // покидание TooltipDialog
            if (leave) {
              anim_out(id);
              setTimeout(function() {
                popup.close(select_search_engines_tt);
              }, 400);
            }
          },
          onMouseUp: function(e) {
            // событие onMouseUp на TooltipDialog
            e = e || event;
            target = e.target || e.srcElement;
            if (target.id == "select_search_engines_done") {
              dojo
                .query("input", search_engines_dialog_tt)
                .forEach(function(item) {
                  var aspect = item.id.substring(0, item.id.indexOf("_"));
                  options.search_engines[aspect] = item.value;
                });
            } else if (target.id == "refresh_search_options")
              options = eval("(" + voice.options + ")"); // восстановим значения options по умолчанию
        ...
        on(menu_item, "mouseover", function(e) {
          ...
          target = e.target || e.srcElement;
          setTimeout(function() {
            if ($(target).is(":hover")) on_menu_item();
          }, 400);
        });
      ...
      require(["dijit/popup"], function(popup) {
        var target,
          menu_item = dojo.byId("the_context");
        ...
        function on_menu_item() {
          ...
          if (!select_context_tt_was) {
            ...
            dojo.query(
              ...
              e = e || event;
              if (e.keyCode == 13) handle_site_upload(false, this);
            };
            dojo.query(
              the_sharp +
                context_dialog_tt_id +
      ...
      require(["dijit/popup"], function(popup) {
        var target,
          menu_item = dojo.byId("the_pareto");
      ...
      require(["dijit/popup"], function(popup) {
        var target,
          menu_item = dojo.byId("the_levenstain");
        ...
        var select_levenstain_tt = new TooltipDialog({
          ...
          onMouseLeave: function() {
            ...
            if (leave) {
              anim_out(id);
              setTimeout(function() {
                popup.close(select_levenstain_tt);
              }, 400);
            }
          },
          onMouseUp: function(e) {
  ...
  var semantic_pane_manipulator = function(semantic_type) {
    ...
    var skrepka_bind = function() {
      $(".skrepka_float", self)
        .unbind("mouseup")
        .bind("mouseup", panel_fixed);
      $(".skrepka_fixed", self)
        .unbind("mouseup")
        .bind("mouseup", panel_float);
    };
    var semantic_scroll_alive = function() {
      if (skrepka_fixed.css("visibility") == "visible") {
        ...
        inner.css({
          height: outer.outerHeight(true) - invite.outerHeight(true) + "px"
        });
      } else {
        if (repetitor_handle) clearInterval(repetitor_handle);
        repetitor_handle = setInterval(function() {
          inner.css({
            height: invite.outerHeight(true) + result.outerHeight(true) + "px"
          });
        }, 200);
      }
    ...
    var panel_fixed = function() {
      ...
      outer
        .append(invite)
        .append(skrepka_float)
        .append(skrepka_fixed)
        .append(inner);
  ...
  function selector_alive(semantic_type, invite) {
    ...
    if (semantic_type == "dbpedia")
      $select.unbind("mouseup").bind("mouseup", function() {
        writer($select.val());
      });
  ...
  function agentIsOK(agent, name) {
    setTimeout(function() {
      if (
        ...
        typeof agent.closed == "undefined"
      )
        browserMessage(voice.mes_agent);
      else {
        agent.blur();
        if (agent && name) win_opened.push({ win: agent, name: name });
      }
    }, 1500);
  }
  ...
  function childIsOK(name) {
    setTimeout(function() {
      if (
        ...
        browserMessage(voice.mes1 + voice.mes2);
      else child.focus();
    }, 800);
...
function browserMessage(mess) {
  ...
  anim({
    ...
    easing: bounceOut,
    properties: {
      top: { start: 0, end: height - 160 },
      opacity: { start: 0.1, end: 1 }
    },
    duration: 1000
  })
    .stop()
...
function writer(n) {
  $(".queryText").val(queries[n].sparql);
}
